...
let%component make = (~onDigit, ~onDelete, ()) => {
	let%hook (v, dispatch) = Hooks.reducer(~initialState={ref: None, hasFocus: false}, reducer);
	...
	let onKeyDown = (event: NodeEvents.keyEventParams) => {
		if(!event.repeat && Key.Keycode.digit0 <= event.keycode && event.keycode <= Key.Keycode.digit9) {
			onDigit(int_of_string(Key.Scancode.getName(event.scancode)));
		}
		else if(event.keycode == Key.Keycode.delete || event.keycode == Key.Keycode.backspace) {
			onDelete();
		}
	};
